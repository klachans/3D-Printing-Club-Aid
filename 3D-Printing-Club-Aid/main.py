...
# hardware-related code is commented
...
# page containing progress bars and other information
@app.route('/')
def index():
...
# page where user can add printers to database
...
# page where user can add filaments to database
...
def fetch_progress():
    for progress_bar in progress_bars:
        ...
        # hardware-related code, displays remaining printing time 
        # on MAX7129 display and gets data from DHT11 sensor
        # to display it on webpage
...
# page where user can end printing
...
def reset_printer():
    ...
    if request.method == 'POST':
        ...
        outcome = "success" if checkbox else "failure" ## ternary operator uwu
        for progress_bar in progress_bars:
